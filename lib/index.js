...
const token = "xoxb-162999408277-6GfxSCAdl7f5fArF0V2WpghJ";
let inRetro = false;
let guid = 0;
let users = [];
let done = [];
...
function userCapture(message){
  ...
  if(!users.includes(user)){
    ...
    if(message.text === 'done'){
        ...
        reply(message, "Your doneness has been noted.");
        notifyWaitingCount(message);
      if(users.length === done.length){
        message.channel = channel;
        waiting_on = users.length - done.length;
        reply(message, "Waiting on " + waiting_on + " users");
        stopRetro(message);
      }
    }
  };
...
function notifyWaitingCount(message){
    waiting_on = users.length - done.length;
    reply(message.channel, "Waiting on " + waiting_on + " users");
}
...
function command(message) {
  ...
  if (aliases[cmd] === start) {
    ...
    let msg = '🔔 Retrospective is now recording. And the Chizeler is waiting quietly for your answers.';
...
function stopRetro(message) {
  ...
  inRetro = false;
  reply(message, 'The Chizeler is processing your answers, give me a second or ' +
